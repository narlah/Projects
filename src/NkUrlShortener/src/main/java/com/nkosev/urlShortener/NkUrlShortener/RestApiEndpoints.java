...
public class RestApiEndpoints {
    ...
    public URLShortenedResponse postURLForShortening(@RequestParam(name = "url") String shortUrl) {
        ...
        if (urlRepo.existsById(shortUrl)) {
            ...
            urlShortenedResponse.setOriginalUrl(((URLKeyValPair) urlOptional.get()).originalUrl);
        } else {
    ...
    public URLShortenedResponse postURLForShortening(@RequestBody URLShortThisRequest shortThisRequest) {
        ...
        List<URLKeyValPair> list = urlRepo.findByOriginalUrl(shortThisRequest.getRequestThis());
        if (list.size() == 0)
            urlRepo.save(new URLKeyValPair(urlShort.shortenedUrl, urlShort.originalUrl));
        else
            urlShort.setShortenedUrl(list.get(0).shortenedUrl);
        return urlShort;
    }

    @RequestMapping(value = "/all", method = RequestMethod.GET)
    public List<URLKeyValPair> getAll() {
        Iterable<URLKeyValPair> iterable = urlRepo.findAll();
        List<URLKeyValPair> result = new ArrayList<>();
