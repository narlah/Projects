...
class RandomizeEngine {
  ...
  void randomizeDir(boolean isFolderSelected, final JProgressBar progressBar) throws IOException {
    ...
    for (int i = 0; i < vectorLenght; i++) {
      ...
      if (newNamedFile.exists()) {
        System.out.println(
            "This file already exists, will not be renamed " + newNamedFile.getAbsolutePath());
      } else {
        File origNamedFile = new File(startDirectory + "//" + originalName);
        // Logging each line with before and after names and paths
        String[] tempList = {origNamedFile.getAbsolutePath(), newNamedFile.length() + "",
        ...
        if (success) {
          ...
          String[] tempListErr = {"There was a problem with renaming the file  ", "0",
              originalName};
  ...
  /**
   * Must remove the numbers in front of the song name to add our numbers. Gets the position of the
   * first letter, ignores numbers and punctuation. Returns new name
   *
