 ...
 * from standard input, either one bit at a time (as a <tt>boolean</tt>), 8 bits at a time (as a
 * <tt>byte</tt> or <tt>char</tt>), 16 bits at a time (as a
...
public final class BinaryStdIn {
  ...
  /**
   * Read the next 8 bits from standard input and return as an 8-bit char. Note that <tt>char</tt>
   * is a 16-bit type; to read the next 16 bits as a char, use <tt>readChar(16)</tt>
   *
   ...
   * <tt>String</tt>
   * @throws RuntimeException if standard input is empty or if the number of bits available on
   ...
   *
   * @return the next 64 bits of data from standard input as a <tt>double</tt> //@throws
   * RuntimeExceptionArgument           if there are fewer than 64 bits available on standard input
   */
