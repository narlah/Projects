...
class BTreePrinter {
  ...
  private static <T extends Comparable<?>> void printNodeInternal(List<Node> nodes, int level,
      int maxLevel) {
  ...
  private static <T extends Comparable<?>> int maxLevel(Node node) {
    ...
    return Math.max(BTreePrinter.maxLevel(node.left) + 1, BTreePrinter.maxLevel(node.right));
  }
