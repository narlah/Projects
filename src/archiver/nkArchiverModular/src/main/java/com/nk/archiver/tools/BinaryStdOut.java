 ...
 * sequences of bits and writing them to standard output. Uses big-endian (most-significant byte
 * first).
 * <p>
...
public final class BinaryStdOut {
  ...
  /**
   * Flush and close standard output. Once standard output is closed, you can no longer write bits
   * to it.
   */
  public void close() {
    flush();
  ...
  public void write(char x) {
    if (x < 0 || x >= 256) {
      throw new IllegalArgumentException("Illegal 8-bit char = " + x + " " + (int) x);
    }
    writeByte(x);
  }
