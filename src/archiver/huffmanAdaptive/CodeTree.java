...
public final class CodeTree {
    ...
    // For example, if symbol 5 has code 10011, then codes.get(5) is the list
    // [1, 0, 0, 1, 1].
    ...
    public CodeTree(InternalNode root, int symbolLimit) {
        ...
        for (int i = 0; i < symbolLimit; i++)
            codes.add(null);
        buildCodeList(root, new ArrayList<Integer>()); // Fills 'codes' with
        // appropriate data
    }
    ...
    public List<Integer> getCode(int symbol) {
        if (symbol < 0) throw new IllegalArgumentException("Illegal symbol");
        else if (codes.get(symbol) == null) throw new IllegalArgumentException("No code for given symbol");
        else
            return codes.get(symbol);
    }

    // Returns a string showing all the codes in this tree. The format is
